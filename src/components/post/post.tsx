...
import { propsPost } from "../types/type";
import { optPost } from "../types/type";
import { PostType } from "../types/type";

const Post = ({ searchPost, BtnSortPost, postRedner }: propsPost) => {
  const menuRef: any = useRef();
  const [showOpt, setshowopt] = useState<optPost>({ id: "", show: false });
  ...
  const SortPost = () => {
    if (BtnSortPost.new && searchPost === "") {
      ...
      return Posts.filter((item) =>
        item.title.toLocaleLowerCase().includes(searchPost.toLocaleLowerCase())
      );
    } else {
      return Posts;
    }
          ...
          <li key={id}>
            <div className={styles.PostTop}>
              ...
              <span className={styles.PostOpen} ref={menuRef}>
                ...
                {showOpt.show && showOpt.id === post.id ? (
                  <div className={styles.PostOpenPanel}>
                    <Link
                      to={`/SocialBook/post/${post.id}`}
                      onClick={() => setshowopt({ id: post.id, show: false })}
                    >
                      Open
                    </Link>
            ...
            <div className={styles.PostBottom}>
              ...
              <button
                ...
                className={
                  post.like.indexOf(user.email) !== -1
                    ? styles.PostAddLikeactive
                    : styles.PostAddLike
                }
              >
                <AiFillLike /> {post.like.length}
              </button>
            </div>
          </li>
        ))
