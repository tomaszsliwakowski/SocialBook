...
import { ComType, PostType } from "../types/type";
    ...
    <>
      {PostContainer.length ? (
        PostContainer.map((item) => (
          <div key={item.id} className={styles.SinglePostBody}>
            ...
            <div className={styles.SinglePostCom}>
              <div className={styles.SinglePostAddCom}>
                <textarea
                  placeholder={
                    user.email ? "Write Comment..." : "Sign in to add a comment"
                  }
                  value={ComInput}
                ...
                <span>
                  ...
                  <button
                    ...
                    className={
                      item.like.indexOf(user.email) !== -1
                        ? styles.SinglePostAddLikeactive
                        : styles.SinglePostAddLike
                    }
                  >
                  ...
                  ? item.com.map((el, id) => (
                      <li key={id}>
                        <div className={styles.com_user}>
                          ...
                          user.username === "admin" ? (
                            <AiOutlineDelete
                              onClick={() => deleteCom(el, item)}
                              className={styles.com_svg}
                            />
                          ) : null}
                        </div>
                        <div className={styles.com_text}>{el.comment}</div>
                      </li>
                    ))
                  : null}
              </ul>
